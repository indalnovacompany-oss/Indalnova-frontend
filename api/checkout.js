...
export default async function handler(req, res) {
  try {
    ...
    if (lines.length < 2) return res.status(500).json({ success: false, message: "Sheet has no products" });
    ...
    for (const item of cart) {
      ...
      if (!product) return res.status(400).json({ success: false, message: `Product ID ${item.id} not found` });
      total += product.price * Number(item.qty);
    }
    if (total <= 0) return res.status(400).json({ success: false, message: "Total amount invalid" });
