...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!user) {
      ...
      const password = userData.password || "default123"; // default password
      const hashedPassword = await bcrypt.hash(password, 10);
      user = new User({ ...userData, password: hashedPassword });
      await user.save();
    } else {
      // Update address if new or changed
      let updated = false;
      ["address","landmark","city","state","pincode","phone","name"].forEach(field => {
        if(user[field] !== userData[field] && userData[field]) {
