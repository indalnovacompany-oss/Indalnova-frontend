...
export default async function handler(req, res) {
  ...
  // Get email from query (GET/DELETE) or body (POST/PUT)
  ...
  if (req.method === "PUT") {
    ...
    try {
      ...
      const coupons = lines.slice(1).map(line => {
        ...
        return headers.reduce((obj, h, i) => { obj[h.trim()] = cols[i].trim(); return obj; }, {});
      });

      const found = coupons.find(c => c.code.toUpperCase() === coupon.toUpperCase());
      if (!found) return res.json({ valid: false, message: "Invalid coupon" });

      // Check if user already used coupon
      if (!user.usedCoupons) user.usedCoupons = [];
      if (user.usedCoupons.includes(coupon.toUpperCase())) {
        return res.json({ valid: false, message: "Coupon already used" });
      }

      user.usedCoupons.push(coupon.toUpperCase());
      await user.save();
